...
class ToyExecutor:
    ...
    def run_task(self, task_name: str, args: List[Any], depth=0):
        ...
        # Parameter binding: parametri scritti in global_memory (NO scope locale)
        # Questo impedisce la ricorsione: ogni chiamata sovrascrive i parametri
        for param, val in zip(task.params, args):
             ...
             # Scrivi direttamente in global_memory
             self.global_memory[param.name] = val
             self.symbol_table_types[param.name] = param.type_name
        
        return self.execute_body(task.body, depth)

    def execute_body(self, body: BodyNode, depth: int = 0):
        for stat in body.statements:
            res = self.execute_stat(stat, depth)
            if res is not None:
                return res
        return None

    def execute_stat(self, stat: StatNode, depth: int = 0):
        if isinstance(stat, AssignmentNode):
            val = self.evaluate_expr(stat.expr, depth)
            # Tutte le variabili sono globali
            if stat.target in self.global_memory:
            ...
            arg_vals = [self.evaluate_expr(a, depth) for a in stat.args]
            self.run_task(stat.task_name, arg_vals, depth + 1)
             ...
             val = self.evaluate_expr(stat.expr, depth)
             # Converti booleani Python in yes/no (semantica Toy-Agent)
             ...
             cond = self.evaluate_expr(stat.condition, depth)
             if cond:
                 return self.execute_body(stat.then_body, depth)
             else:
                 for elif_b in stat.elif_blocks:
                      if self.evaluate_expr(elif_b.condition, depth):
                           return self.execute_body(elif_b.body, depth)
                 if stat.else_body:
                      return self.execute_body(stat.else_body, depth)

        elif isinstance(stat, LoopNode):
             while self.evaluate_expr(stat.condition, depth):
                 res = self.execute_body(stat.body, depth)
                 if res: return res

    def evaluate_expr(self, expr: ExprNode, depth: int = 0) -> Any:
        if isinstance(expr, AtomNode):
            return expr.value
        elif isinstance(expr, VarUsageNode):
            # Solo global_memory, no scope locale
            if expr.name in self.global_memory: return self.global_memory[expr.name]
            raise RuntimeError(f"Undefined variable: {expr.name}")
        elif isinstance(expr, BinaryOpNode):
            l = self.evaluate_expr(expr.left, depth)
            r = self.evaluate_expr(expr.right, depth)
            ...
            v = self.evaluate_expr(expr.expr, depth)
            if expr.op == 'not': return not v
        elif isinstance(expr, TaskCallExprNode):
            args = [self.evaluate_expr(a, depth) for a in expr.args]
            return self.run_task(expr.task_name, args, depth + 1)
